...
function formatActivityTime(activity: ItineraryActivity): string {
...
export default function ItineraryScreen() {
  ...
  const loadItinerary = useCallback(async () => {
    ...
    try {
      ...
      const parsed: unknown = storedTrips ? JSON.parse(storedTrips) : [];
      const trips = Array.isArray(parsed) ? (parsed as SavedTrip[]) : [];
  ...
  const openAddActivity = (
    date: string,
    period: ItineraryTimePeriod = "morning"
  ) => {
    if (!tripId) {
      return;
    }
    ...
    router.push({
      ...
      params: { tripId, date, period },
    });
  };

  const openEditActivity = (activity: ItineraryActivity) => {
    if (!tripId) {
      return;
    }
    ...
    router.push({
      ...
      params: { tripId, activityId: activity.id },
    });
  };

  const confirmDeleteActivity = (activity: ItineraryActivity) => {
    if (!tripId) {
      return;
    }
  ...
  const renderActivity = (activity: ItineraryActivity) => (
    <Pressable
      key={activity.id}
    ...
    >
      ...
      {activity.address ? (
        <Text style={styles.activityDetail}>🗺️ {activity.address}</Text>
      ) : null}
  ...
  const renderPeriod = (
    ...
    const details = PERIOD_DETAILS[period];
      ...
      <View key={period} style={styles.periodSection}>
        <View style={styles.periodHeader}>
          <View style={styles.periodHeading}>
            <Text style={styles.periodEmoji}>{details.emoji}</Text>
            <Text style={styles.periodTitle}>{details.title}</Text>
          </View>

          <Pressable
            onPress={() => openAddActivity(day.date, period)}
            style={({ pressed }) => [
        ...
        {activities.length > 0 ? (
          ...
          >
            <Text style={styles.emptyPeriodText}>
              No {details.title.toLowerCase()} plans yet
            </Text>
            <Text style={styles.emptyPeriodAction}>Add an activity</Text>
          </Pressable>
        )}
      </View>
    );
  ...
  if (isLoading) {
    return (
      <SafeAreaView style={styles.centeredContainer}>
        ...
        <Text style={styles.loadingText}>Building your itinerary...</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.safeArea} edges={["bottom"]}>
      ...
      >
        <View style={styles.heroCard}>
          <Text style={styles.heroEyebrow}>YOUR ITINERARY</Text>
        ...
        {itinerary.days.length > 0 ? (
          ...
          <View style={styles.noDaysCard}>
            <Text style={styles.noDaysEmoji}>📅</Text>
            <Text style={styles.noDaysTitle}>No itinerary days</Text>
            <Text style={styles.noDaysDescription}>
              Check that your trip has valid start and end dates.
            </Text>
          </View>
        )}
      </ScrollView>
      ...
      {itinerary.days.length > 0 ? (
        <View style={styles.floatingButtonContainer}>
          <Pressable
            onPress={() => openAddActivity(itinerary.days[0].date)}
            style={({ pressed }) => [
...
const styles = StyleSheet.create({
  ...
  periodHeading: { flexDirection: "row", alignItems: "center" },
  periodEmoji: { marginRight: 7, fontSize: 17 },
  periodTitle: {
    fontSize: 15,
    fontWeight: "800",
    color: "#413A36",
  ...
  noDaysEmoji: { marginBottom: 15, fontSize: 42 },
  noDaysTitle: {
    marginBottom: 7,
    fontSize: 20,
    fontWeight: "800",
  ...
  loadingText: { marginTop: 13, fontSize: 15, color: "#746C68" },
  notFoundEmoji: { marginBottom: 16, fontSize: 46 },
  notFoundTitle: {
    marginBottom: 8,
    fontSize: 23,
    fontWeight: "800",
